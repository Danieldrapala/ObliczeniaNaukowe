...
# wejscie:	f - ścieżka do pliku z macierzą n l i potem i j A[i,j]
# wyjscie (A,n,l):
#		A - macierz rzadka wczytana z pliku, 
#		n - rozmiar macierzy,
#		l - wielkość bloku
function load_matrix_ffile(f::String)
	open(f) do file
		...
		n = parse(Int64, ln[1])
		l = parse(Int64, ln[2])
		el_num = n*l + 3*(n-l) #wielkosc potrzebna do przechowania el niezerowych
		...
		while !eof(file)
			ln = split(readline(file))
			I[it] = parse(Int64, ln[1])
			J[it] = parse(Int64, ln[2])
